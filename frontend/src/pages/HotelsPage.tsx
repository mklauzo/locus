...
import {
  ...
  FormControlLabel, IconButton, Alert, CircularProgress,
} from '@mui/material';
...
const emptyHotel = {
  name: '', address: '', email: '', imap_host: '', imap_port: 993,
  imap_ssl: true, imap_login: '', imap_password: '',
};
  ...
  return (
    <>
      <Typography variant="h5" sx={{ mb: 3, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        Hotele
        <Button variant="contained" startIcon={<Add />} onClick={() => { setForm(emptyHotel); setEditId(null); setOpen(true); }}>
          Dodaj hotel
        </Button>
      </Typography>
      ...
      <Dialog open={open} onClose={() => setOpen(false)} maxWidth="sm" fullWidth>
        ...
        <DialogContent sx={{ display: 'flex', flexDirection: 'column', gap: 2, pt: '16px !important' }}>
          ...
          <Typography variant="subtitle2" sx={{ mt: 1 }}>Konfiguracja IMAP</Typography>
          <TextField label="Host IMAP" value={form.imap_host} onChange={e => setForm({ ...form, imap_host: e.target.value })} fullWidth />
          <TextField label="Port" type="number" value={form.imap_port} onChange={e => setForm({ ...form, imap_port: +e.target.value })} />
          <FormControlLabel control={<Checkbox checked={form.imap_ssl} onChange={e => setForm({ ...form, imap_ssl: e.target.checked })} />} label="SSL" />
          ...
          <Button
            ...
            startIcon={imapTesting ? <CircularProgress size={18} /> : undefined}
            onClick={async () => {
              setImapTest(null);
              setImapTesting(true);
              try {
                ...
                setImapTest({ type: 'error', text: err.response?.data?.message || 'Błąd połączenia' });
              } finally {
